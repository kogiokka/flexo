...
bool Shader::Attach(GLenum shaderType, std::string_view const filepath)
{
    using namespace std;
...
int Shader::UniformLocation(std::string_view const uniformName)
{
...
void Shader::SetUniformMatrix4fv(std::string_view const name, glm::mat4 const& mat)
{
    glUniformMatrix4fv(UniformLocation(name), 1, GL_FALSE, glm::value_ptr(mat));
}

void Shader::SetUniform3f(std::string_view const name, float x, float y, float z)
{
    glUniform3f(UniformLocation(name), x, y, z);
}

void Shader::SetUniform3fv(std::string_view const name, glm::vec3 const& vec)
{
    glUniform3fv(UniformLocation(name), 1, glm::value_ptr(vec));
}

void Shader::SetUniform3fv(std::string_view const name, std::array<float, 3> const& vec)
{
    glUniform3fv(UniformLocation(name), 1, vec.data());
}

void Shader::SetUniform1f(std::string_view const name, float value)
{
    glUniform1f(UniformLocation(name), value);
}

void Shader::SetUniform1i(std::string_view const name, int value)
{
...
bool Shader::IsCompiled(GLuint const shaderObject)
{
    GLint success;
